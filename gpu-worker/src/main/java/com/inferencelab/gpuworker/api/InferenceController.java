...
public class InferenceController {
    ...
    private void handleSubmit(Context ctx) {
        ...
        InferenceSubmitResponse response = new InferenceSubmitResponse(
            requestId,
            "QUEUED",
            "/api/inference/" + requestId + "/stream"
        );
        ctx.json(response);
    }
    ...
    private void handleGetStatus(Context ctx) {
        ...
        ctx.status(404).json(java.util.Map.of("error", "Not implemented", "request_id", requestId));
    }
    ...
    private void handleStream(SseClient client) {
        ...
        client.sendEvent("meta", "{\"request_id\":\"" + requestId + "\",\"status\":\"QUEUED\"}");
        client.close();
    }
}
